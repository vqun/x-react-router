...
var RouterRender = function (_Component) {
  ...
  _createClass(RouterRender, [{
    ...
    key: 'renderElements',
    value: function renderElements(Routing, key) {
      var els = [];
      for (var k = Routing.length; k--;) {
        els.unshift(this.renderElement(Routing[k], key + '.' + k));
      }
      return els;
    }
  }, {
    ...
    value: function renderElement(routing, key) {
      ...
      return render({
        routing: root,
        children: childRoutings && this.renderElements(childRoutings, key),
        key: key
      });
    }
  }]);
...
RouterRender.defaultProps = {
  ...
  render: function render(props) {
    return _react2.default.createElement(_RoutingRender2.default, props);
  }
};
