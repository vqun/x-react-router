...
assign(Routing, (_assign = {}, _defineProperty(_assign, lutings, []), _defineProperty(_assign, 'compile', function compile(routes) {
  var parent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : { originalPath: '/' };

  var routings = this[lutings];
  var _newRoutings = createRoutings(routes, parent);
  var L = _newRoutings.length;
  var newAdded = [];
  for (var k = 0; k < L; k++) {
    var newRouting = _newRoutings[k];
    this.disposeRouting(newRouting, routings) && newAdded.push(newRouting);
  }
...
}), _defineProperty(_assign, 'disposeRouting', function disposeRouting(routing, routings) {
  ...
  for (var k = routings.length; k--;) {
    ...
    if (isSame && typeof component === 'string') {
      ...
      this.disposeRouting(routing, childRoutings || []);
    }
  }
